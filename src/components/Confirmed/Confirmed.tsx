...
import type { ConfirmedRaid, RaidType, RaidGroup, RaidMember, ClassType } from '../../lib/types';
...
function MemberCard({ member }: { member: RaidMember }) {
  ...
  return (
    ...
    >
      <span className={`px-1.5 py-0.5 rounded text-xs font-bold shrink-0 ${CLASS_BADGE[member.class_type]}`}>
        {member.class_type}
      </span>
      <span
  ...
  return (
    <div className="border border-gray-200 dark:border-gray-600 rounded-lg p-4 bg-white dark:bg-gray-800 shadow-sm">
      <div className="flex items-center justify-between mb-3 flex-wrap gap-1">
        <h3 className="text-lg font-bold text-gray-900 dark:text-gray-100">공격대 {raid.id}</h3>
...
export default function Confirmed() {
  const [selectedWeek, setSelectedWeek] = useState(() => formatDate(getWednesday(new Date())));
      ...
      <div className="mb-4">
        ...
        <div className="flex gap-2">
          {RAID_TYPES.map(rt => (
            <button
              key={rt}
              onClick={() => { setSelectedRaid(rt); setConfirmed(null); }}
            ...
            <div>
              ...
              <div className="flex items-center justify-between mb-4 p-3 bg-green-50 dark:bg-green-900/30 border border-green-300 dark:border-green-700 rounded-lg">
                <div className="flex items-center gap-2">
                  ...
                  <span className="font-bold text-green-800 dark:text-green-300">공격대 확정됨</span>
